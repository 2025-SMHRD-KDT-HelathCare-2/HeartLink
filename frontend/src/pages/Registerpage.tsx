...
import { Heart, Eye, EyeOff, Lock, Mail, User, Phone, Shield, ArrowLeft } from "lucide-react";
...
export function RegisterPage({ onRegister, onGoLogin }: RegisterPageProps) {
  ...
  const [form, setForm] = useState({
    email: "", password: "", name: "", phone: "", guardianCode: "", agree: false,
  });
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0A2647] via-[#144272] to-[#0E8080] flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-white/80 mt-2" style={{ fontSize: "1.2rem" }}>회원가입</p>
        </div>

        <div className="bg-white rounded-2xl shadow-2xl p-8">
          {/* 뒤로가기 */}
          <button onClick={onGoLogin} className="flex items-center gap-2 text-gray-500 hover:text-[#0A2647] font-bold mb-6 transition-colors" style={{ fontSize: "1rem" }}>
            ...
            {role === "guardian" && (
              <div>
                <label className="block text-gray-700 mb-2 font-bold" style={{ fontSize: "1.1rem" }}>
                  연결 코드 <span className="text-gray-400 font-normal">(선택)</span>
                </label>
                <input type="text" placeholder="어르신에게 받은 연결 코드" value={form.guardianCode}
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5">
              <label className="flex items-start gap-4 cursor-pointer">
                ...
                <span className="text-amber-800 leading-relaxed font-bold" style={{ fontSize: "1.05rem" }}>
                  개인정보 수집·이용에 동의합니다. (필수)<br />
                  <span className="text-amber-700 font-normal">심장 건강 분석을 위한 건강 정보 포함</span>
                </span>
              </label>
              {errors.agree && <p className="text-red-500 mt-2 font-bold" style={{ fontSize: "1rem" }}>{errors.agree}</p>}
            </div>
